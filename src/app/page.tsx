...
import {
  ...
  useSensors,
  DragEndEvent,
  UniqueIdentifier,        // ← NEW: import this
} from "@dnd-kit/core";
import {
  arrayMove,
  SortableContext,
...
// FIXED TYPES — this is the only real change needed!
type Task = {
  id: UniqueIdentifier;        // ← string | number (dnd-kit expects this)
  title: string;
  description?: string;
  column: UniqueIdentifier;    // ← also allow string | number here
};
  ...
  return (
    <Card
      ...
      className={`p-4 rounded-xl shadow-lg cursor-grab active:cursor-grabbing touch-none
        ...
        ${
          ["1", "3", "4", "5"].includes(task.id as string)
...
export default function Home() {
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    // 1. Dropped directly on a column header (todo/inprogress/done)
    if (over.id === "todo" || over.id === "inprogress" || over.id === "done") {
      setTasks((tasks) =>
        tasks.map((t) => (t.id === activeTask.id ? { ...t, column: over.id } : t))
      );
      return;
    }
        ...
        setTasks((tasks) =>
          tasks.map((t) =>
            t.column === activeTask.column
              ? reorderedColumn.find((nt) => nt.id === t.id) ?? t
              : t
          )
        );
      }
          ...
          <div className="max-w-7xl mx-auto px-6 pb-20">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <div className="bg-white/60 dark:bg-white/5 backdrop-blur-xl rounded-3xl shadow-2xl p-8 border border-white/20">
                ...
                <SortableContext
                  items={tasks.filter((t) => t.column === "todo").map((t) => t.id)}
              ...
              <div className="bg-white/60 dark:bg-white/5 backdrop-blur-xl rounded-3xl shadow-2xl p-8 border border-white/20">
                ...
                <SortableContext
                  items={tasks.filter((t) => t.column === "inprogress").map((t) => t.id)}
                  strategy={verticalListSortingStrategy}
                >
              ...
              <div className="bg-white/60 dark:bg-white/5 backdrop-blur-xl rounded-3xl shadow-2xl p-8 border border-white/20">
                ...
                <SortableContext
                  items={tasks.filter((t) => t.column === "done").map((t) => t.id)}
                  strategy={verticalListSortingStrategy}
                >
